<x-app-layout>
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div class="bg-white shadow-md rounded-lg p-6">
            <h2 class="text-2xl font-semibold text-gray-700 mb-4">Edit Intake</h2>
            ...
            <form action="{{ route('intakes.update', $intake->id) }}" method="POST">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" id="name" name="name" value="{{ old('name', $intake->name) }}" 
                ...
                <div class="mb-4">
                    ...
                    <input type="date" id="start_date" name="date" value="{{ old('start_date', $intake->date) }}" 
                        class="w-full px-4 py-2 border rounded-lg focus:ring focus:ring-blue-300" required>
                </div>

               {{--  <div class="mb-4">
                    <label for="end_date" class="block text-gray-600">End Date</label>
                    <input type="date" id="end_date" name="end_date" value="{{ old('end_date', $intake->end_date) }}" 
                        class="w-full px-4 py-2 border rounded-lg focus:ring focus:ring-blue-300" required>
                </div> --}}

                <div class="mb-4">
                    <label for="status" class="block text-gray-600">Status</label>
                    <select id="status" name="registration_enabled" 
                        class="w-full px-4 py-2 border rounded-lg focus:ring focus:ring-blue-300">
                        <option value="1" {{ $intake->registration_enabled ? 'selected' : '' }}>Active</option>
                        <option value="0" {{ !$intake->registration_enabled ? 'selected' : '' }}>Inactive</option>
                    </select>
                </div>
